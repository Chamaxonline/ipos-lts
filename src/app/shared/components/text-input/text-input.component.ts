...
export class TextInputComponent implements ControlValueAccessor {
  ...
  getErrorText() {
    ...
    if (this.ngControl.control?.errors?.['email']) return 'Please Enter Email';
    ...
    if (this.ngControl.control?.errors?.['min']) return 'Invalid range';
    if (this.ngControl.control?.errors?.['max']) return 'Invalid range';
    if (this.ngControl.control?.errors?.['pattern']) return 'Invalid format';
    if (this.ngControl.control?.errors?.['whitespace']) return 'Required';
    return '';
  }
}
